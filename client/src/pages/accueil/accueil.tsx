...
const Accueil = () => {
  ...
  // Partenaire sélectionné via la carte
  const [selectedPartenaire, setselectedPartenaire] = useState('');
  const [selectedPartenaireError, setselectedPartenaireError] = useState('Veuillez sélectionner un partenaire via la carte.')

  // Info utilisateur + erreur
  const [name, setName] = useState('');
  const [nameError, setNameError] = useState('Le nom ne doit contenir que des lettres et des espaces.');
  const [firstname, setFirstname] = useState('');
  const [firstnameError, setFirstnameError] = useState('Le prénom ne doit contenir que des lettres et des espaces.');
  const [email, setEmail] = useState('');
  const [emailError, setEmailError] = useState('L\'adresse e-mail n\'est pas valide.');

  // Infos partenaire qui sont affichés dans les champs
  const [partenairename, setpartenairename] = useState('');
  const [partenairecp, setpartenairecp] = useState('');
  const [partenaireaddresse, setpartenaireaddresse] = useState('');
  const [partenaireville, setpartenaireville] = useState('');
  // Id du partenaire envoyé dans le form
  ...
  // Tout les types d'activitée
  const [options, setOptions] = useState<OptionType[]>([
    { value: '', label: 'Tout' },
    { value: 'Parc Terroir', label: 'Parc Terroir' },
    { value: 'Vélo', label: 'Vélo' },
    { value: 'Ecomusée', label: 'Ecomusée' },
    { value: 'Château', label: 'Château' },
    { value: 'Laser Game', label: 'Laser Game' },
    { value: 'Jeu de piste', label: 'Jeu de piste' },
    { value: 'Hockey sur Gazon', label: 'Hockey sur Gazon' },
    { value: 'Boxe Française', label: 'Boxe Française' },
    { value: 'Escrime', label: 'Escrime' },
    { value: 'Jardin Botanique', label: 'Jardin Botanique' },
    { value: 'Cours de guitare', label: 'Cours de guitare' },
    { value: 'Cours de danse', label: 'Cours de danse' },
    { value: 'Char à Voile', label: 'Char à Voile' },
    { value: 'Cours de piano', label: 'Cours de piano' },
  ]);
  const [selectedOption, setSelectedOption] = useState<OptionType | null>(options[0]);

  // Facture
  const [uploadedFile, setUploadedFile] = useState<File | null>(null);
  const [fileError, setfileError] = useState('Veuillez déposer une preuve d\'achat valide (.png, .jpg, .gif, .pdf) de 10Mo max.')

  // Filtre des types d'activitée
  const filteredPartenaires = partenaires.filter((partenaire) => {
    if (selectedOption && selectedOption.value !== '') {
      return (
        partenaire.type.toLowerCase().includes(selectedOption.value.toLowerCase())
      );
    }
    return true;
  });
  ...
  useEffect(() => {
      const fetchData = async () => {
          try {
              const allPartenaires = await api.getAllPartenaires();
              setPartenaires(allPartenaires);

              // Reset the style for the previously selected region
              const previousRegionElement = document.getElementById(previousSelectedRegion);
              if (previousRegionElement) {
                  previousRegionElement.style.fill = ""; // Reset to default or another color
              }

              const regionElement = document.getElementById(selectedRegion);
              if (regionElement) {
                  regionElement.style.fill = "rgb(23,39,89)";
              }

              // Update the previous selected region
...
const partenairenameHandler = (e:any) => {
    setpartenairename(e.target.value);
    validateName();
}

const partenairecpHandler = (e:any) => {
    setpartenairecp(e.target.value);
}

const handleVilleChange = (e:any) => {
  setpartenaireville(e.target.value);
}
...
const validateName = () => {
  if (!/^[A-Za-zÀ-ÖØ-öø-ÿ][A-Za-zÀ-ÖØ-öø-ÿ\s]*$/.test(name)) {
    setNameError('Le nom ne doit contenir que des lettres et des espaces.');
  } else {
    setNameError('');
  }
};
...
const validateEmail = () => {
  ...
  const validateFirstname = () => {
    if (!/^[A-Za-zÀ-ÖØ-öø-ÿ][A-Za-zÀ-ÖØ-öø-ÿ\s]*$/.test(firstname)) {
      setFirstnameError('Le prénom ne doit contenir que des lettres et des espaces.');
    } else {
      setFirstnameError('');
    }
  };
...
const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && (file.type === 'image/png' || file.type === 'image/jpeg' || file.type === 'application/pdf' || file.type === 'image/gif' ) && file.size < (10 * 1024 * 1024)){
        setUploadedFile(file);
        setfileError('');
    } else {
        alert('Déposez un fichier PNG, JPG, GIF ou PDF de 10Mo maximum.');
        setUploadedFile(null);
        setfileError('Veuillez déposer une preuve d\'achat valide (.png, .jpg, .gif, .pdf) de 10Mo max.');
    }
}
...
const handleRegionClick = async (region:any) => {
    try {
      ...
      // On récupere les partenaires de la région sélectionnée
      const partenairesByRegion: Partenaire[] = await api.getPartenairesByRegion(region);
      setPartenaires(partenairesByRegion);
  
      console.log(partenaires)
      setSelectedRegion(region);
      const uniqueTypes = Array.from(new Set(partenairesByRegion.map((p: any) => p.type)));

      // Créer de nouvelles options en incluant 'Tout'
      ...
      // Si le type sélectionné n'est plus disponible, réinitialiser le selectedOption
      if (!newOptions.some((option) => option.value === selectedOption?.value)) {
        setSelectedOption(newOptions[0]); // Réinitialiser à 'Tout'
      }
    } catch (error) {
        console.error(`Error fetching partenaires by region (${region}):`, error);
    }
};
const isPartenaireSelected = (id:any) => {
    return selectedPartenaire === id;
};
const handleSelection = async (partenaire:any) => {

    // On récupère le partenaire sélectionné et on set l'erreur à vide
    setselectedPartenaire(partenaire);
    setselectedPartenaireError('');

    // On set les champs du partenaire sélectionné
    setpartenaireaddresse(partenaire.adresse);
    setpartenairename(partenaire.nom);
    setpartenairecp(partenaire.cp);
    setpartenaireville(partenaire.ville);
    // On récupère l'Id du partenaire
    setPartenaireId(partenaire.id);
};

const addParticipationHandler = async () => {
  // On regarde si il y a une erreur sur un des champs
  if (nameError || firstnameError || emailError || fileError || selectedPartenaireError){
    // si oui, on alert et on sort
    window.alert(nameError + '\n' +  firstnameError + '\n' + emailError + '\n' + selectedPartenaireError + '\n' + fileError );
    return;
  }

  // Sinon on crée et remplis le form
  const formData = new FormData();

  formData.append('nom', name);
  formData.append('firstname', firstname);
  formData.append('email', email);
  // On ajoute l'Id du partenaire car on envoie pas toutes les informations
  formData.append('partenaireId', PartenaireId);
  if (uploadedFile) {
      formData.append('imageFacture', uploadedFile, uploadedFile.name);
      formData.append('imageFactureType', uploadedFile.type);
  }
  try {
    navigate("../merci");
    const result = await api.addParticipation(formData);
    console.log('Participation added successfully:', result);
    } catch (error) {
        ...
        <div className='flex-grow md:flex md:h-screen'>
          <div className="flex-1 basis-full mx-1">
            <svg
              ...
              version="1.1"
              viewBox="0 0 615 585"
              className="w-full h-full font-roboto font-extrabold text-[10px] [fill:rgb(29,68,127)] [stroke:rgb(8,26,73)] [stroke-width:1]"
            >
              <a
                id="Auvergne-Rhône-Alpes"
                title="Auvergne-Rhône-Alpes"
                onClick={() => handleRegionClick('Auvergne-Rhône-Alpes')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Auvergne-Rhône-Alpes' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a
                id="Bourgogne-Franche-Comté" title="Bourgogne-Franche-Comté"
                onClick={() => handleRegionClick('Bourgogne-Franche-Comté')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Bourgogne-Franche-Comté' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a
                id="Bretagne" title="Bretagne" onClick={() => handleRegionClick('Bretagne')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Bretagne' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a id="Centre-Val de Loire" title="Centre-Val de Loire"
                onClick={() => handleRegionClick('Centre-Val de Loire')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Centre-Val de Loire' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a id="Corse" title="Corse" onClick={() => handleRegionClick('Corse')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Corse' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a id="Grand Est" title="Grand Est" onClick={() => handleRegionClick('Grand Est')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Grand Est' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a id="Hauts-de-France" title="Hauts-de-France"
                onClick={() => handleRegionClick('Hauts-de-France')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Hauts-de-France' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a id="Île-de-France" title="Île-de-France"
                onClick={() => handleRegionClick('Île-de-France')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Île-de-France' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a
                id="Normandie" title="Normandie" onClick={() => handleRegionClick('Normandie')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Normandie' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a
                id="Nouvelle-Aquitaine" title="Nouvelle-Aquitaine"
                onClick={() => handleRegionClick('Nouvelle-Aquitaine')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Nouvelle-Aquitaine' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a
                id="Occitanie" title="Occitanie" onClick={() => handleRegionClick('Occitanie')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Occitanie' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a
                id="Pays de la Loire" title="Pays de la Loire"
                onClick={() => handleRegionClick('Pays de la Loire')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Pays de la Loire' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
              ...
              <a
                id="Provence-Alpes-Côte d'Azur" title="Provence-Alpes-Côte d'Azur"
                onClick={() => handleRegionClick('Provence-Alpes-Côte d\'Azur')}
                className={`cursor-pointer hover:[fill:rgb(28,48,89)] ${selectedRegion === 'Provence-Alpes-Côte d\'Azur' ? '[fill:rgb(23,39,89)]' : ''}`}
              >
          ...
          <div className="flex flex-col mx-6 md:mx-10 basis-full rounded-3xl border-2 border-white max-h-screen">
            <Select
              ...
              onChange={(newValue: SingleValue<OptionType>) => setSelectedOption(newValue)}
              options={options}
              styles={customStyles}
              ...
              isSearchable={false}
            />
            <ul className="flex flex-col space-y-2 p-3 md:px-8 md:pb-4 overflow-y-auto">
              {filteredPartenaires.map((partenaire, index) => (
                <div key={index} className="flex bg-white rounded-lg shadow-md p-4 items-center gap-4 justify-center">
                  <div className="flex-grow-0 items-center justify-center">
                    ...
                    <button
                    onClick={() => handleSelection(partenaire)}
                    className={` mt-3
                  px-4 py-2 items-center justify-center rounded-lg transition-colors duration-200
                  whitespace-pre-line min-w-[100px]
                  ${isPartenaireSelected(partenaire.id)
                        ? 'bg-white text-black border-2 border-black'
                        : 'bg-black text-white hover:bg-gray-800'}
                `}
                  >
                  <a href='#Formulaire'>Sélectionner</a>
                  </button>
                  </div>
                </div>
              ))}
